...
各章节沿用开场的 Nebula Dark 风格，开场叙事为在架空空间醒来、面前有许多扇门。点击画面或按 Enter/Space 进入下一句。教程不设逐项教学，只在左侧显示按键表，Gemini 默认折叠。所有章节开局就显示完整地图，叠加自己的位置和路径，右侧保留指南针。教程隐藏全部声源；追逐隐藏电机和其他玩家；旅馆显示完整房间、门、楼梯、人物、推车和录音机，抵达楼上即显示整层。旅馆先拆开信封阅读案件，再进入调查；支持相对方向提示和可随时接管的短程导航协助。上楼使用用户指定音频的前 3 秒，上楼动作同为 3 秒，下楼保留原声。
