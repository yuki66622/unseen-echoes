...
export function drawFloorPlan(ctx,{scene,floor,walls,door,bounds,doors,sources,X,Y,scale,pixelScale}) {
  const markers=[];
  ...
  ctx.stroke();
  ctx.font=`400 ${11*pixelScale}px system-ui`;ctx.textAlign='center';ctx.fillStyle='#77889d';
  for(const room of ROOMS){if(room.floor===floor)ctx.fillText(room.label,X(room.x),Y(room.y));}
  ...
  ctx.stroke();ctx.textAlign='center';ctx.fillStyle='#9fb1c6';ctx.fillText('Stairs',X(16),Y(11.5));

  ctx.font=`400 ${11*pixelScale}px system-ui`;
  for(const item of LANDMARKS){
    ...
    ctx.textAlign=item.align||'left';ctx.fillText(item.label,X(p.x)+item.dx*pixelScale,Y(p.y)+item.dy*pixelScale);
  }
  return markers;
}
