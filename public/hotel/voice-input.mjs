...
const browserTransport = {
  getStatus: ({ signal }) => requestJson('/api/hotel/status', { method: 'GET' }, signal),
  transcribe: (input, { signal, token, status, context }) => {
    if(status.understanding){
      ...
      return requestJson('/api/hotel/interpret',{method:'POST',headers:{'Content-Type':'application/json','X-Voice-Token':token},
    ...
    return requestJson('/api/hotel/transcribe', {
