...
const title=document.createElement('span');title.textContent=isTutorial?'初次穿行 · 找到雨声':'第二关 · Pinewood Inn';
...
if(isTutorial){
  ...
  const finishLink=document.createElement('a');finishLink.className='journey-finish';finishLink.textContent='返回中转站';finishLink.href=nextURL('/','title');finishLink.onclick=()=>{try{sessionStorage.removeItem('unseen-checkpoint');}catch{}};ending.append(finishLink);
