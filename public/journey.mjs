...
const title=document.createElement('span');title.textContent=isTutorial?'初次穿行':'第二关 · Pinewood Inn';
...
if(isTutorial){
  ...
  const finishLink=document.createElement('a');finishLink.className='journey-finish';finishLink.textContent='返回门前';finishLink.href=nextURL('/','title');finishLink.onclick=()=>{try{sessionStorage.removeItem('unseen-checkpoint');}catch{}};ending.append(finishLink);
