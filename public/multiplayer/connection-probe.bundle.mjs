...
// connection-probe.ts
function probeRoomService(config) {
  return new Promise((resolve) => {
    const started = performance.now();
    let conn, done = false, stage = "websocket";
    const finish = (ok, code) => {
      ...
      resolve({ ok, stage, code, ms: Math.round(performance.now() - started) });
    };
    const timer = setTimeout(() => finish(false, "timeout"), 12e3);
    try {
      conn = DbConnection.builder().withUri(config.uri).withDatabaseName(config.database).withCompression("none").onConnect((active) => {
        if (done) {
          active.disconnect();
          return;
        }
        conn = active;
        stage = "subscription";
        active.subscriptionBuilder().onApplied(() => finish(true, "ready")).onError(() => finish(false, "subscription-rejected")).subscribe(tables.myRoom);
      }).onConnectError(() => finish(false, "handshake-failed")).onDisconnect(() => finish(false, "socket-closed")).build();
      if (done) conn.disconnect();
    } catch {
      finish(false, "browser-unsupported");
    }
  });
}
export {
