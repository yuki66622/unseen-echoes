...
export class NavigationHUD {
  constructor({map = false, scene = 'chase', label = '', walls = [], door = null, mapDoors = [], bounds={width:8,height:8,north:8}} = {}) {
    ...
    if(map) {
      if(scene==='chase'||scene==='world'){this.budget=document.createElement('p');this.budget.id='search-budget';this.budget.hidden=true;this.orientation.append(this.budget);}
      ...
      this.mapHost.querySelector('header span:last-child').textContent=label||(scene==='tutorial'?'初次穿行':'追逐');host.append(this.mapHost);
      this.trail = new TrailMap(this.mapHost.querySelector('canvas'),{scene,walls,door,mapDoors,bounds});
    }
    this.roundKey = null; this.lastMapFrame = 0;
  }
  update({player,active=false,paused=false,roundKey,attemptsRemaining,doorOpen=false,doors=null} = {}) {
    ...
    if(this.trail){
      ...
      if(now-this.lastMapFrame>=50){
        this.trail.update({player:{...player,floor:0},doors:doors||{main:doorOpen?1:0},paused,elapsed:now/1000});this.lastMapFrame=now;
