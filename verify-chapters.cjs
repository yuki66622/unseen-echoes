...
(async()=>{
 ...
 try{
  ...
  await key(a,'w',9);await key(a,'d',3,300);await key(a,'w',2);await key(a,'f',1,50);await key(a,'w',6);await key(a,'e',1,80);
  ...
  await key(a,'w',11,600);await key(a,'e',1,80);await Promise.all([a,b].map(p=>p.locator('#result').waitFor({timeout:8000})));
  pass('real hunter keyboard movement and interaction produce a shared capture result');
  await a.locator('#next-level').click();await a.waitForURL('**/hotel/**');await a.locator('#start').waitFor();
  ...
  await a.locator('#start').click();await a.locator('#chat').waitFor();await a.screenshot({path:out+'/integrated-hotel-play.png'});
