...
(async()=>{
 ...
 try{
  ...
  await keys('ArrowUp',11);await keys('ArrowRight',3);await keys('ArrowUp',8);await keys('f');await page.waitForTimeout(450);
  ...
  const other=await browser.newContext({viewport:{width:1280,height:850},permissions:[],reducedMotion:'reduce'}),b=await other.newPage();
  ...
  await b.screenshot({path:out+'/chase-map.png'});pass('both chase players see the full open-field map immediately; hunter activity does not leak onto the other map');
  for(const p of [page,b]){
   ...
   assert.equal(s.voices.some(v=>v.id==='ending'),false);assert.equal(s.masterGain,0);
  }
  ...
  for(const p of [page,b]){
   ...
   assert.equal(s.voices.some(v=>v.id==='background'||v.id==='heartbeat'),false);assert.equal(s.masterGain,0);
  }
